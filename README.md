...
키 이름 통합은 기본적으로 꺼져 있습니다. 같은 대상을 파일마다 다른 명칭으로 기록한 경우에만 비교 설정의 **키 이름 통합 사용**을 켭니다. 매핑 파일도 비교 파일과 마찬가지로 브라우저 내부에서만 읽으며 서버로 업로드하거나 영구 저장하지 않습니다. 매핑은 하나만 만들며, A·B 양쪽 파일에서 선택한 키 컬럼에 같은 매핑을 적용합니다. 컬럼 이름이 파일마다 달라 적용 대상만 양쪽에서 각각 선택합니다.
...
빈 별칭 셀은 무시합니다. 대표값 자체도 자기 자신으로 등록합니다. 매핑에 없는 값은 오류가 아니며 기존 정규화 값 그대로 비교합니다.

### 직접 등록과 JSON 재사용

소규모 매핑은 화면에서 **그룹 추가** 후 대표값과 쉼표로 구분한 별칭을 입력합니다. 그룹은 수정·삭제할 수 있습니다. 현재 매핑은 다음 구조의 JSON으로 저장하고 다시 불러올 수 있습니다.
