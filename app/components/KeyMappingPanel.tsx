        ...
        <div className="key-mapping-panel__body">
          ...
          {table ? (
            <>
              ...
              <details className="diagnostics"><summary>원본 Wide 미리보기 · {table.rows.length.toLocaleString('ko-KR')}행</summary><div className="result-table-wrap"><table><thead><tr>{table.headers.map(header => <th key={header}>{header}</th>)}</tr></thead><tbody>{table.rows.slice(0, 20).map((row, rowIndex) => <tr key={rowIndex}>{table.headers.map((header, columnIndex) => <td key={header}>{String(decodeScalar(row[columnIndex]))}</td>)}</tr>)}</tbody></table></div></details>
            </>
          ) : null}
