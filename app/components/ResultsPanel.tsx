...
export default function ResultsPanel({ result }: { result?: ComparisonResult }) {
  ...
  const allFirstRowsSingle = validRows.length > 0 && validRows.every(row => row.aCount === 1);
  ...
  return <section className="results-panel">
    ...
    <div className="result-table-wrap"><table><thead><tr><th>Lane 키</th><th>일치 여부</th><th>{allFirstRowsSingle ? '두 번째 시트 매칭 건수' : '첫/두 번째 시트 행 수'}</th><th>첫 번째 시트 비교값</th><th>두 번째 시트 비교값</th></tr></thead><tbody>{rows.map((row, index) => <tr key={index}><td>{row.key.map(value => String(decodeScalar(value))).join(' / ')}</td><td>{statusLabels[row.status] ?? row.displayStatus}</td><td>{allFirstRowsSingle ? row.bCount.toLocaleString('ko-KR') : `${row.aCount.toLocaleString('ko-KR')} / ${row.bCount.toLocaleString('ko-KR')}`}</td><td>{comparisonValues(row, 'A')}</td><td>{comparisonValues(row, 'B')}</td></tr>)}</tbody></table></div>
