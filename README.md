...
키 이름 통합은 기본적으로 꺼져 있습니다. 같은 대상을 파일마다 다른 명칭으로 기록한 경우에만 비교 설정의 **키 이름 통합 사용**을 켭니다. 매핑 파일도 비교 파일과 마찬가지로 브라우저 내부에서만 읽으며 서버로 업로드하거나 영구 저장하지 않습니다. 사전은 하나만 만들며, A·B 양쪽 파일에서 선택한 키 컬럼에 같은 사전을 적용합니다. 컬럼 이름이 파일마다 달라 적용 대상만 양쪽에서 각각 선택합니다.
